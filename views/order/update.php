                            ...
                            <div class="card-body">
                                <form class="form" method="post" enctype="multipart/form-data">
                                    <div class="card-block">
                                        <div class="card-body">
                                            <p class="text-muted">Mã hóa đơn</p>
                                            <fieldset class="form-group">
                                                <input type="text" class="form-control" id="readonlyInput" readonly="readonly" value="<?php echo $data['order']['Ma_HoaDon']?>">
                                            </fieldset>
                                        </div>
                                    </div>
                                    <div class="form-body">
                                        <div class="form-group">
                                            <label for="basicSelect" class="pl-2 h5">Mã khách hàng</label>
                                            <input type="text" name="customerid" class="form-control" readonly="readonly" value="<?php echo $data['order']['MaKhachHang']?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="donationinput3" class="pl-2 h5">Tên người nhận</label>
                                            <input type="text" name="customername" id="donationinput3" class="form-control" placeholder="Tên người nhận" value="<?php echo $data['order']['TenNguoiNhan']?>">
                                        </div>
                                        <div class="form-group">
                                            <label  class="pl-2 h5">Tên sản phẩm</label>
                                            <?php for($i = 0; $i < $count; $i++):?>
                                                <select name="product[]" id="#" class="form-control my-1">
                                                    <?php foreach($data['products'] AS $key => $value):?>
                                                        <option value="<?php echo $value['TenSanPham']?>" <?php if($value['TenSanPham']==$orders[$i]) {echo "selected";}?>>
                                        ...
                                        <div class="form-group">
                                            <label for="donationinput2" class="pl-2 h5">Địa chỉ giao hàng</label>
                                            <input type="text" name="address" id="donationinput2" class="form-control" placeholder="Địa chỉ giao hàng" value="<?php echo $data['order']['DiaChiGiaoHang']?>">
                                        </div>
                                    </div>
